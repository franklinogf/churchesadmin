...
interface ICalendarContext {
  ...
  updateEvent: (eventId: IEvent['id'], updatedEvent: Partial<IEvent>) => void;
  removeEvent: (eventId: number) => void;
  clearFilter: () => void;
  setEvents: (updatedEvents: IEvent[]) => void;
}
...
export function CalendarProvider({
  ...
  const filterEventsBySelectedUser = (userId: IUser['id'] | 'all') => {
    ...
    if (userId === 'all') {
      ...
      const filtered = allEvents.filter((event) => event.creator?.id === userId);
      setFilteredEvents(filtered);
    }
  };
  ...
  const updateEvent = (eventId: IEvent['id'], updatedEvent: Partial<IEvent>) => {
    const updated = { ...allEvents.find((e) => e.id === eventId), ...updatedEvent } as IEvent;

    setAllEvents((prev) => prev.map((e) => (e.id === eventId ? updated : e)));
    setFilteredEvents((prev) => prev.map((e) => (e.id === eventId ? updated : e)));
  };

  const setEvents = (updatedEvents: IEvent[]) => {
