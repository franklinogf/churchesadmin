...
export function getEventsCount(events: IEvent[], date: Date, view: TCalendarView): number {
  ...
  return events.filter((event) => compareFn(parseISO(event.startAt), date)).length;
}

export function groupEvents(dayEvents: IEvent[]): IEvent[][] {
  const sortedEvents = dayEvents.sort((a, b) => parseISO(a.startAt).getTime() - parseISO(b.startAt).getTime());
  const groups: IEvent[][] = [];
  ...
  for (const event of sortedEvents) {
    ...
    for (const group of groups) {
      ...
      const lastEventEnd = parseISO(lastEventInGroup!.endAt);
...
export function getEventBlockStyle(event: IEvent, day: Date, groupIndex: number, groupSize: number) {
  ...
  const eventStart = startAt < dayStart ? dayStart : startAt;
...
export function calculateMonthEventPositions(multiDayEvents: IEvent[], singleDayEvents: IEvent[], selectedDate: Date): Record<string, number> {
  ...
  const sortedEvents = [
    ...multiDayEvents.sort((a, b) => {
      const aDuration = differenceInDays(parseISO(a?.endAt), parseISO(a.startAt));
      const bDuration = differenceInDays(parseISO(b?.endAt), parseISO(b.startAt));
      return bDuration - aDuration || parseISO(a.startAt).getTime() - parseISO(b.startAt).getTime();
    }),
    ...singleDayEvents.sort((a, b) => parseISO(a.startAt).getTime() - parseISO(b.startAt).getTime()),
  ];
...
export function getMonthCellEvents(date: Date, events: IEvent[], eventPositions: Record<string, number>) {
  ...
  return eventsForDate
    .map((event) => ({
      ...
      isMultiDay: event.startAt !== event?.endAt,
    }))
    .sort((a, b) => {
...
export const getEventsForDay = (events: IEvent[], date: Date, isWeek = false): IEvent[] => {
  ...
  return events
    .filter((event) => {
      const startOfDayForEventStart = startOfDay(parseISO(event.startAt));
      const startOfDayForEventEnd = startOfDay(parseISO(event?.endAt));
      if (isWeek) {
        return event.startAt !== event?.endAt && startOfDayForEventStart <= targetDate && startOfDayForEventEnd >= targetDate;
      }
    ...
    .map((event) => {
      ...
      const eventEnd = startOfDay(parseISO(event?.endAt));
...
export const getWeekDates = (date: Date): Date[] => {
  const startAt = startOfWeek(date, { weekStartsOn: 1 });
  return Array.from({ length: 7 }, (_, i) => addDays(startAt, i));
};
