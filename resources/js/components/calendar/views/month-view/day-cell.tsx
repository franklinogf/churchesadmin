...
export function DayCell({ cell, events, eventPositions }: IProps) {
  ...
  const renderEventAtPosition = useCallback(
    (position: number) => {
      ...
      const showBullet = isSameMonth(new Date(event.startAt), currentCellMonth);
  ...
  const cellContent = useMemo(
    () => (
      <motion.div
        className={cn('flex h-full flex-col gap-1 border-t border-l lg:min-h-40', isSunday(date) && 'border-l-0')}
        ...
        <DroppableArea date={date} className="h-full w-full py-2">
          <motion.span
            className={cn(
              ...
              isToday(date) && 'bg-primary text-primary-foreground flex w-6 translate-x-1 items-center justify-center rounded-full px-0 font-bold',
            )}
          >
            {day}
          </motion.span>

          <motion.div className={cn('mt-1 flex h-fit gap-1 px-2 lg:h-23.5 lg:flex-col lg:gap-2 lg:px-0', !currentMonth && 'opacity-50')}>
            {cellEvents.length === 0 && !isMobile ? (
              <div className="group flex h-full w-full items-center justify-center">
                <AddEditEventDialog startDate={date}>
                  <Button variant="ghost" className="border opacity-0 transition-opacity duration-200 group-hover:opacity-100">
