...
export function MonthEventBadge({ event, cellDate, eventCurrentDay, eventTotalDays, className, position: propPosition }: IProps) {
  ...
  const itemStart = startOfDay(parseISO(event.startAt));
  const itemEnd = endOfDay(parseISO(event.endAt));
    ...
    <DraggableEvent event={event} className={marginClass}>
      <EventDetailsDialog event={event}>
        <button type="button" className={eventBadgeClasses}>
          ...
          <div className="hidden sm:block">{renderBadgeTime && <span>{formatTime(new Date(event.startAt), use24HourFormat)}</span>}</div>
        </button>
      </EventDetailsDialog>
    </DraggableEvent>
  );
