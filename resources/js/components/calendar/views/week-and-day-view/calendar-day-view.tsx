        ...
        <div>
          ...
          <div className="relative z-20 flex border-b">
            ...
            <span className="text-t-quaternary flex-1 border-l py-2 text-center text-xs font-medium">
              {format(selectedDate, 'EE')} <span className="text-t-secondary font-semibold">{format(selectedDate, 'd')}</span>
            </span>
          </div>
        </div>
              ...
              <div className="relative">
                {hours.map((hour, index) => (
                  <div key={hour} className="relative" style={{ height: '96px' }}>
                    ...
                    <DroppableArea date={selectedDate} hour={hour} minute={0} className="absolute inset-x-0 top-0 h-12">
                    ...
                    <DroppableArea date={selectedDate} hour={hour} minute={30} className="absolute inset-x-0 bottom-0 h-12">
            ...
            <ScrollArea className="h-105.5 px-4" type="always">
              <div className="space-y-6 pb-4">
                {currentEvents.map((event) => {
                  const user = users?.find((user) => user.id === event.creator?.id);
                  ...
                  return (
                    <div key={event.id} className="space-y-1.5">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-t-tertiary text-sm">{format(new Date(event.startAt), 'MMM d, yyyy')}</span>
                      </div>

                      <div className="flex items-center gap-1.5">
                        <Clock className="text-t-quinary size-4" />
                        <span className="text-t-tertiary text-sm">
                          {format(parseISO(event.startAt), use24HourFormat ? 'HH:mm' : 'hh:mm a')} -
                          {format(parseISO(event.endAt), use24HourFormat ? 'HH:mm' : 'hh:mm a')}
                        </span>
                      </div>
                    </div>
                  );
