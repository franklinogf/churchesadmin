...
it('can delete an expense and its transaction', function (): void {
    ...
    expect(Expense::query()->find($expense->id))->not->toBeNull()
        ->and(Transaction::query()->find($expense->transaction_id))->not->toBeNull();
    ...
    expect(Expense::query()->find($expense->id))->toBeNull()
        ->and(Transaction::query()->find($expense->transaction_id))->toBeNull();
...
it('deletes expense in a transaction to ensure consistency', function (): void {
    ...
    expect(Expense::query()->find($expenseId))->toBeNull()
        ->and(Transaction::query()->find($transactionId))->toBeNull();
});
