    ...
    public function show(Request $request): HttpResponse
    {
        ...
        return Pdf::loadView('pdf.entries_expenses', [
            'title' => __('Entries and Expenses - :month1 :year1 to :month2 :year2', [
                'month1' => Date::parse($startOfMonth)->translatedFormat('F'),
                'year1' => Date::parse($startOfMonth)->format('Y'),
                'month2' => Date::parse($endOfMonth)->translatedFormat('F'),
                'year2' => Date::parse($endOfMonth)->format('Y'),
            ]),
            'entries' => $entries,
            'expenses' => $expenses,
            'dates' => $dates,
            ...
            ->stream('entries_expenses_'.Date::parse($startOfMonth)->format('Y_m').'.pdf');
    }
